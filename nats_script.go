...
func main() {
	...
	nc, err := stan.Connect("cluster_1", "client_2", stan.NatsURL("nats://localhost:4222"))
	if err != nil {
		log.Fatal(err)
	}
	defer nc.Close()
	...
	for {
		...
		// Publish the order to NATS Streaming
		err = nc.Publish("orders", orderJSON)
		if err != nil {
			log.Println("Error publishing message:", err)
			continue
		}

		fmt.Println("Message sent:", string(orderJSON))
